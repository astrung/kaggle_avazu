...
def extract_time_stamp_feature(data_frame):
    ...
    data_frame['hour'] = data_frame['hour'].apply(lambda x: x % 100)  # hour
    return data_frame
...
if __name__ == "__main__":
    ...
    mlp = MLPClassifier(hidden_layer_sizes=(10), alpha=1e-4,
                        ...
                        , max_iter=1000
                        , random_state=1
                        , learning_rate='adaptive', batch_size=10000)
    reduce_features = SelectKBest(f_classif)
    pipeline = Pipeline([
        ('rf', reduce_features),
        ('mlp', mlp)
    ])
    ...
    parameters = {
        'rf__k': (30, 40, 50),
        'mlp__batch_size': (500, 1000, 2000,5000),
        'mlp__learning_rate_init': (0.001, 0.002, 0.005, 0.008),
        'mlp__learning_rate': ('adaptive', 'invscaling')
    }
    log_loss_build = metrics.make_scorer(metrics.log_loss, greater_is_better=False, needs_proba=True)
    grid_search = GridSearchCV(pipeline, parameters, n_jobs=-1, cv=4, scoring=log_loss_build)
